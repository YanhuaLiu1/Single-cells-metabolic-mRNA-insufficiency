...
df = tdfread('Data/RNAseq_data_with_entrez_genes_average(max_score)_Huh1_7_Patient.csv'); % RNA-seq
...
datasetnames = {'Huh1','Huh7','Patient'};
datasets = {Huh1, Huh7, Patient};
...
% newObjectiveReactionID = 'LDH_D'; 
% newObjectiveReactionIndex = find(strcmp(R3.rxns, newObjectiveReactionID));
% newObjectiveVector = zeros(size(R3.c));
% newObjectiveVector(newObjectiveReactionIndex) = 1;
% R3.c = newObjectiveVector;
...
final_NNR_index = 27; % was 13
final_NNR = scales(final_NNR_index);

% store results in a matrix
% columns for the NNR factor, rows for the 18 models
% values will be the maximal biomass fluxs
...
for j = 1:length(datasets)
    ...
    RAScolumn = RASmatrix(:,j); % Huh1, Huh7, Patient
...
semilogx(scales,results(1:6,:),'--','LineWidth', 2) % Huh1
hold on;
ax = gca;
...
legend('Huh1 M1','Huh1 M2','Huh1 M3','Huh1 M4','Huh1 M5','Huh1 M6',...
       'Huh7 M1','Huh7 M2','Huh7 M3','Huh7 M4','Huh7 M5','Huh7 M6',...
       'Patient M1','Patient M2','Patient M3','Patient M4','Patient M5','Patient M6','Location', 'eastoutside')
line([final_NNR, final_NNR],[0,0.7],'Color','black','LineStyle','--','HandleVisibility','off')
hold off;
...
xticks(1:18)
xticklabels({'Huh1 M1','Huh1 M2','Huh1 M3','Huh1 M4','Huh1 M5','Huh1 M6',...
             'Huh7 M1','Huh7 M2','Huh7 M3','Huh7 M4','Huh7 M5','Huh7 M6',...
             'Patient M1','Patient M2','Patient M3','Patient M4','Patient M5','Patient M6'})
xtickangle(45)
ylabel('Maximal biomass flux')
...
% results_min = zeros(18, length(objectives));
% results_max = zeros(18, length(objectives));
% model_count = 1;
% for cell_line = {'Huh1','Huh7','Patient'}
%     for medium = {'1','2','3','4','5','6'}
...
%     plot([1:18; 1:18],[(results_min(:,i))'; results_max(:,i)'], 'LineWidth',5,'Color','k')
%     hold on;
%     for j = 1:18 % loop over models for current objective i and if range is small show a marker
%         if abs(results_min(j,i) - results_max(j,i)) < 1e-1
%             plot(j, results_max(j,i),'*k')
%         end
%     end
%     xlim([0,19]) % for extra space on the RHS
